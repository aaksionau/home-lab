...
resource "kubernetes_secret_v1" "postgres" {
  ...
  data = {
    ...
    POSTGRES_DB       = "stocks"
    # config.py reads DATABASE_URL directly (psycopg connection string).
    DATABASE_URL = "postgresql://stocks:${random_password.postgres.result}@postgres:5432/stocks"
  }
}
...
resource "kubernetes_persistent_volume_claim_v1" "postgres" {
  ...
  spec {
    ...
    resources {
      requests = {
        # Snapshots accumulate indefinitely (all 500 tickers, daily, no
        # pruning) -- sized generously up front so this doesn't need
        # revisiting soon after launch.
        storage = "20Gi"
      }
    }
  }
    ...
    template {
      ...
      spec {
        container {
          ...
          readiness_probe {
            exec {
              command = ["pg_isready", "-U", "stocks", "-d", "stocks"]
            }
